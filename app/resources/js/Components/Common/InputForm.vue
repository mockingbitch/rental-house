<template>
    <div :class="`request__form-wrap ${isError ? 'error' : ''}`">
        <div class="d-flex">
            <label>
                {{ label }}
                <i
                    v-if="isDisplayIcon"
                    class="with-tooltip d-flex align-items-center"
                    :data-tooltip-content="
                        lang().label.information.account_info.country_tooltip
                    "
                >
                    <img src="/img/icon/Question.svg" alt="" />
                </i>
            </label>
            <span v-if="isShowIconLeft" class="icon-select-left">
                <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M10 1.875C8.39303 1.875 6.82214 2.35152 5.486 3.24431C4.14985 4.1371 3.10844 5.40605 2.49348 6.8907C1.87852 8.37535 1.71762 10.009 2.03112 11.5851C2.34463 13.1612 3.11846 14.6089 4.25476 15.7452C5.39106 16.8815 6.8388 17.6554 8.4149 17.9689C9.99099 18.2824 11.6247 18.1215 13.1093 17.5065C14.594 16.8916 15.8629 15.8502 16.7557 14.514C17.6485 13.1779 18.125 11.607 18.125 10C18.1227 7.84581 17.266 5.78051 15.7427 4.25727C14.2195 2.73403 12.1542 1.87727 10 1.875ZM16.8461 9.375H13.7344C13.6242 7.15156 12.932 5.04141 11.7719 3.35938C13.1306 3.72417 14.3452 4.49613 15.2523 5.57152C16.1594 6.64691 16.7156 7.97419 16.8461 9.375ZM10 16.8664C8.53672 15.2828 7.64922 13.0383 7.51797 10.625H12.482C12.3508 13.0367 11.4633 15.2828 10 16.8664ZM7.51797 9.375C7.64922 6.96328 8.5336 4.71719 10 3.13359C11.4633 4.71719 12.3508 6.96172 12.482 9.375H7.51797ZM8.22813 3.35938C7.06797 5.04141 6.37578 7.15156 6.26563 9.375H3.15391C3.28444 7.97419 3.84063 6.64691 4.74773 5.57152C5.65483 4.49613 6.86937 3.72417 8.22813 3.35938ZM3.15391 10.625H6.26563C6.37578 12.8484 7.06797 14.9586 8.22813 16.6406C6.86937 16.2758 5.65483 15.5039 4.74773 14.4285C3.84063 13.3531 3.28444 12.0258 3.15391 10.625ZM11.7719 16.6406C12.932 14.9563 13.6242 12.8461 13.7344 10.625H16.8461C16.7156 12.0258 16.1594 13.3531 15.2523 14.4285C14.3452 15.5039 13.1306 16.2758 11.7719 16.6406Z"
                        fill="#B1B1B1"
                    />
                </svg>
            </span>
            <span v-if="isRequiredInput" class="required-input">*</span>
        </div>
        <!-- <input type="text" v-model="message" :placeholder="placeholder" /> -->
        <input
            :class="{ input_with_icon: isShowIconLeft }"
            type="text"
            :name="name"
            :value="attrs?.message"
            :placeholder="placeholder"
            @input="message = $event.target.value"
            @blur="onBlur($event.target.name)"
            :disabled="disable"
        />
        <span v-if="isShowTextRight" class="icon-select-right">
            {{ isShowTextRight }}
        </span>
    </div>
</template>
<script setup>
import { ref, defineProps, defineEmits, watch, useAttrs } from "vue";

const {
    disable = false,
    label,
    name = "",
    isError = false,
    placeholder,
    isShowIconLeft,
    isShowTextRight,
    isRequiredInput = false,
} = defineProps([
    "disable",
    "label",
    "name",
    "isError",
    "placeholder",
    "isDisplayIcon",
    "isShowIconLeft",
    "isShowTextRight",
    "isRequiredInput",
]);
const attrs = useAttrs();
const message = ref(attrs?.message ?? "");
const emit = defineEmits();

watch(message, (value) => {
    emit("update:message", value);
});
const onBlur = (field) => {
    emit("onBlur", field);
};
</script>

<style lang="scss" scoped>
.request__form-wrap {
    position: relative;

    &.error {
        label {
            color: #e71a41;
        }

        input {
            border-color: #e71a41;
        }
    }
}
.icon-select-right,
.icon-select-left {
    position: absolute;
    z-index: 2;
    transform: translateY(-45%);
}
@media screen and (min-width: 320px) and (max-width: 767px) {
    .icon-select-right {
        top: 65%;
        right: 2.5%;
        color: #1b1b1b;
    }
    .icon-select-left {
        top: 70%;
        left: 2.5%;
        color: #b1b1b1;
    }
}
@media screen and (min-width: 768px) {
    .icon-select-right {
        top: 70%;
        right: 2.5%;
        color: #1b1b1b;
    }
    .icon-select-left {
        top: 76%;
        left: 2.5%;
        color: #b1b1b1;
    }
}
.input_with_icon {
    padding-left: 35px;
    padding-right: 80px;
}
label {
    display: flex;
    gap: 2px;
}
:deep::placeholder {
    color: #b1b1b1;
    opacity: 1; /* Firefox */
}

:deep:-ms-input-placeholder {
    /* Internet Explorer 10-11 */
    color: #b1b1b1;
}

:deep::-ms-input-placeholder {
    /* Microsoft Edge */
    color: #b1b1b1;
}

.required-input {
    color: red;
    font-size: 14px;
}
</style>
