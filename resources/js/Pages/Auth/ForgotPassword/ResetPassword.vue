...
const form = useForm({
    ...
    email: props.emailGet,
    token: props.tokenGet,
})


const submit = () => {
    form.post(route('reset-password-confirm'), {
        onFinish: () => {
            form.reset('password'),
        ...
        <div class="signin2">
            <div class="container">
                <form class="form" @submit.prevent="submit">
                    <div class="form__wrap form__wrap-password">
                        <h2 class="title form__wrap-password-title">
                            {{ lang().label.reset_password.reset_password }}
                        </h2>
                        <ul class="content">
                            <li>{{ lang().label.reset_password.enter_new_password }}</li>
                        </ul>
                        
                        <div class="form__wrap-item" :class="{ error: form.errors.password }">
                            <label for="Password">
                                {{ lang().label.reset_password.field.password }}
                            </label>
                            <div class="inputWrap">
                                <input :type="showPassword ? 'text' : 'password'"
                                    v-model="form.password"
                                    name="password"
                                    :placeholder="lang().label.reset_password.field.password">
                                <div @click="showPassword = !showPassword" class="icon">
                                    <i v-if="showPassword">
                            ...
                            <ul class="note">
                                <li>※ {{ lang().label.reset_password.hint }}</li>
                                <li>※ {{ lang().label.reset_password.note }}</li>
                            </ul>
                        </div>

                        <div class="form__wrap-item" :class="{ error: form.errors.password_confirmation }">
                            <label for="ConfirmPassword">
                                {{ lang().label.reset_password.field.password_confirmation }}
                            </label>
                            <div class="inputWrap">
                                <input :type="showPasswordConfirm ? 'text' : 'password'"
                                    v-model="form.password_confirmation"
                                    name="password_confirmation"
                                    :placeholder="lang().label.reset_password.field.password_confirmation">
                                <div @click="showPasswordConfirm = !showPasswordConfirm" class="icon">
                                    <i v-if="showPasswordConfirm">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="23" height="16" viewBox="0 0 23 16" fill="none">
                                            <path d="M11.4306 0.915283C6.52031 0.915283 2.3269 3.9695 0.62793 8.28076C2.3269 12.592 6.52031 15.6462 11.4306 15.6462C16.3409 15.6462 20.5344 12.592 22.2333 8.28076C20.5344 3.9695 16.3409 0.915283 11.4306 0.915283ZM11.4306 13.1911C8.72013 13.1911 6.52031 10.9913 6.52031 8.28076C6.52031 5.57026 8.72013 3.37044 11.4306 3.37044C14.1411 3.37044 16.3409 5.57026 16.3409 8.28076C16.3409 10.9913 14.1411 13.1911 11.4306 13.1911ZM11.4306 5.33457C9.8004 5.33457 8.48444 6.65053 8.48444 8.28076C8.48444 9.91098 9.8004 11.2269 11.4306 11.2269C13.0609 11.2269 14.3768 9.91098 14.3768 8.28076C14.3768 6.65053 13.0609 5.33457 11.4306 5.33457Z" fill="#B1B1B1"/>
                                        </svg>
                                    </i>
                            ...
                            <UlError :message="form.errors.password_confirmation" />
                            <ul class="note">
                                <li>※ {{ lang().label.reset_password.hint }}</li>
                                <li>※ {{ lang().label.reset_password.note }}</li>
                            </ul>
                        ...
                        <div>
<!--                            <input type="hidden" value="$page.props.email" v-model="email">-->
<!--                            <input type="hidden" value="$page.props.email" v-model="token">-->
                        </div>
                        <button type="submit" class="mainButton bg-green mt-0" href="#">
                            <p>{{ lang().label.reset_password.setting_completed }}</p>
                        </button>
                    </div>
                </form>
            </div>
