<script setup>
import ImagePreview from "@/Components/ImagePreview/ImagePreview.vue";
import ParentVidPreview from "@/Components/ParentVidPreview/ParentVidPreview.vue";
import ChildVidPreview from "@/Components/ChildVidPreview/ChildVidPreview.vue";
import UlError from "@/Components/UlError.vue";
import { useForm } from "@inertiajs/vue3";

const props = defineProps({
    formLesson: Object,
    message: Object,
});
</script>
<template>
    <div class="form__wrap-item">
        <label for="Title"
            >{{ lang().label.create_lesson.field.cover_image }}
            <span class="required-input">*</span>
        </label>
        <ImagePreview
            :formLesson="props.formLesson"
            :message="formLesson.errors.cover_image"
        />
        <ul class="note">
            <li>※ 画像のサイズ制限</li>
            <li>※ 画像の容量制限</li>
        </ul>
    </div>
    <div class="form__wrap-item">
        <label for="Title">{{ lang().label.create_lesson.field.video_for_kid }} </label>
        <ChildVidPreview
            :formLesson="props.formLesson"
            :message="formLesson.errors.short_video_for_learner"
        />
        <ul class="note">
            <li>※ 60-90秒</li>
        </ul>
    </div>
    <div class="form__wrap-item">
        <label for="Title"
            >{{ lang().label.create_lesson.field.video_for_parent }}
            <span class="required-input">*</span>
        </label>
        <ParentVidPreview
            :formLesson="props.formLesson"
            :message="formLesson.errors.short_video_for_parent"
        />
        <ul class="note">
            <li>※ 60-90秒</li>
        </ul>
    </div>
    <div class="form__wrap-item mb-8">
        <label for="Category">{{ lang().label.create_lesson.field.choose_template }} </label>
        <select name="template" id="" v-model="formLesson.template">
            <option value="1">Template 1</option>
            <option value="2">Template 2</option>
            <option value="3">Template 3</option>
        </select>
        <UlError :message="formLesson.errors.template" />
    </div>
    <div class="form__wrap-item">
        <label for="Category"
            >{{ lang().label.create_lesson.field.lesson_content }}
            <span class="required-input">*</span>
            <span
                >({{
                    formLesson.lesson_content
                        ? formLesson.lesson_content?.length
                        : 0
                }}/10,000)</span
            >
            <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M8 1.5C6.71442 1.5 5.45772 1.88122 4.3888 2.59545C3.31988 3.30968 2.48676 4.32484 1.99479 5.51256C1.50282 6.70028 1.37409 8.00721 1.6249 9.26809C1.8757 10.529 2.49477 11.6872 3.40381 12.5962C4.31285 13.5052 5.47104 14.1243 6.73192 14.3751C7.99279 14.6259 9.29973 14.4972 10.4874 14.0052C11.6752 13.5132 12.6903 12.6801 13.4046 11.6112C14.1188 10.5423 14.5 9.28558 14.5 8C14.4982 6.27665 13.8128 4.62441 12.5942 3.40582C11.3756 2.18722 9.72335 1.50182 8 1.5ZM8 12C7.85167 12 7.70666 11.956 7.58333 11.8736C7.45999 11.7912 7.36386 11.6741 7.30709 11.537C7.25033 11.4 7.23548 11.2492 7.26441 11.1037C7.29335 10.9582 7.36478 10.8246 7.46967 10.7197C7.57456 10.6148 7.7082 10.5434 7.85368 10.5144C7.99917 10.4855 8.14997 10.5003 8.28701 10.5571C8.42406 10.6139 8.54119 10.71 8.6236 10.8333C8.70602 10.9567 8.75 11.1017 8.75 11.25C8.75 11.4489 8.67098 11.6397 8.53033 11.7803C8.38968 11.921 8.19892 12 8 12ZM8.5 8.955V9C8.5 9.13261 8.44732 9.25979 8.35356 9.35355C8.25979 9.44732 8.13261 9.5 8 9.5C7.86739 9.5 7.74022 9.44732 7.64645 9.35355C7.55268 9.25979 7.5 9.13261 7.5 9V8.5C7.5 8.36739 7.55268 8.24021 7.64645 8.14645C7.74022 8.05268 7.86739 8 8 8C8.82688 8 9.5 7.4375 9.5 6.75C9.5 6.0625 8.82688 5.5 8 5.5C7.17313 5.5 6.5 6.0625 6.5 6.75V7C6.5 7.13261 6.44732 7.25979 6.35356 7.35355C6.25979 7.44732 6.13261 7.5 6 7.5C5.86739 7.5 5.74022 7.44732 5.64645 7.35355C5.55268 7.25979 5.5 7.13261 5.5 7V6.75C5.5 5.50938 6.62125 4.5 8 4.5C9.37875 4.5 10.5 5.50938 10.5 6.75C10.5 7.83625 9.64 8.74563 8.5 8.955Z"
                    fill="#B1B1B1"
                />
            </svg>
        </label>
        <textarea
            name="lesson_content"
            rows="5"
            v-model="formLesson.lesson_content"
            maxlength="10000"
        ></textarea>
        <UlError :message="formLesson.errors.lesson_content" />
        <ul class="note">
            <li>※ 学べる内容やゴールについて詳細を記載する</li>
        </ul>
    </div>
    <div class="form__wrap-item">
        <label for="Category"
            >{{ lang().label.create_lesson.field.teacher_experience }}
            <span class="required-input">*</span>
            <span
                >({{
                    formLesson.teacher_experience
                        ? formLesson.teacher_experience?.length
                        : 0
                }}/10,000)</span
            >
            <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M8 1.5C6.71442 1.5 5.45772 1.88122 4.3888 2.59545C3.31988 3.30968 2.48676 4.32484 1.99479 5.51256C1.50282 6.70028 1.37409 8.00721 1.6249 9.26809C1.8757 10.529 2.49477 11.6872 3.40381 12.5962C4.31285 13.5052 5.47104 14.1243 6.73192 14.3751C7.99279 14.6259 9.29973 14.4972 10.4874 14.0052C11.6752 13.5132 12.6903 12.6801 13.4046 11.6112C14.1188 10.5423 14.5 9.28558 14.5 8C14.4982 6.27665 13.8128 4.62441 12.5942 3.40582C11.3756 2.18722 9.72335 1.50182 8 1.5ZM8 12C7.85167 12 7.70666 11.956 7.58333 11.8736C7.45999 11.7912 7.36386 11.6741 7.30709 11.537C7.25033 11.4 7.23548 11.2492 7.26441 11.1037C7.29335 10.9582 7.36478 10.8246 7.46967 10.7197C7.57456 10.6148 7.7082 10.5434 7.85368 10.5144C7.99917 10.4855 8.14997 10.5003 8.28701 10.5571C8.42406 10.6139 8.54119 10.71 8.6236 10.8333C8.70602 10.9567 8.75 11.1017 8.75 11.25C8.75 11.4489 8.67098 11.6397 8.53033 11.7803C8.38968 11.921 8.19892 12 8 12ZM8.5 8.955V9C8.5 9.13261 8.44732 9.25979 8.35356 9.35355C8.25979 9.44732 8.13261 9.5 8 9.5C7.86739 9.5 7.74022 9.44732 7.64645 9.35355C7.55268 9.25979 7.5 9.13261 7.5 9V8.5C7.5 8.36739 7.55268 8.24021 7.64645 8.14645C7.74022 8.05268 7.86739 8 8 8C8.82688 8 9.5 7.4375 9.5 6.75C9.5 6.0625 8.82688 5.5 8 5.5C7.17313 5.5 6.5 6.0625 6.5 6.75V7C6.5 7.13261 6.44732 7.25979 6.35356 7.35355C6.25979 7.44732 6.13261 7.5 6 7.5C5.86739 7.5 5.74022 7.44732 5.64645 7.35355C5.55268 7.25979 5.5 7.13261 5.5 7V6.75C5.5 5.50938 6.62125 4.5 8 4.5C9.37875 4.5 10.5 5.50938 10.5 6.75C10.5 7.83625 9.64 8.74563 8.5 8.955Z"
                    fill="#B1B1B1"
                />
            </svg>
        </label>
        <textarea
            name="teacher_experience"
            rows="5"
            v-model="formLesson.teacher_experience"
            maxlength="10000"
        ></textarea>
        <UlError :message="formLesson.errors.teacher_experience" />
    </div>
    <div class="form__wrap-item mb-8">
        <label for="Category"
            >補足情報
            <span
                >({{
                    formLesson.note ? formLesson.note?.length : 0
                }}/10,000)</span
            >
            <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M8 1.5C6.71442 1.5 5.45772 1.88122 4.3888 2.59545C3.31988 3.30968 2.48676 4.32484 1.99479 5.51256C1.50282 6.70028 1.37409 8.00721 1.6249 9.26809C1.8757 10.529 2.49477 11.6872 3.40381 12.5962C4.31285 13.5052 5.47104 14.1243 6.73192 14.3751C7.99279 14.6259 9.29973 14.4972 10.4874 14.0052C11.6752 13.5132 12.6903 12.6801 13.4046 11.6112C14.1188 10.5423 14.5 9.28558 14.5 8C14.4982 6.27665 13.8128 4.62441 12.5942 3.40582C11.3756 2.18722 9.72335 1.50182 8 1.5ZM8 12C7.85167 12 7.70666 11.956 7.58333 11.8736C7.45999 11.7912 7.36386 11.6741 7.30709 11.537C7.25033 11.4 7.23548 11.2492 7.26441 11.1037C7.29335 10.9582 7.36478 10.8246 7.46967 10.7197C7.57456 10.6148 7.7082 10.5434 7.85368 10.5144C7.99917 10.4855 8.14997 10.5003 8.28701 10.5571C8.42406 10.6139 8.54119 10.71 8.6236 10.8333C8.70602 10.9567 8.75 11.1017 8.75 11.25C8.75 11.4489 8.67098 11.6397 8.53033 11.7803C8.38968 11.921 8.19892 12 8 12ZM8.5 8.955V9C8.5 9.13261 8.44732 9.25979 8.35356 9.35355C8.25979 9.44732 8.13261 9.5 8 9.5C7.86739 9.5 7.74022 9.44732 7.64645 9.35355C7.55268 9.25979 7.5 9.13261 7.5 9V8.5C7.5 8.36739 7.55268 8.24021 7.64645 8.14645C7.74022 8.05268 7.86739 8 8 8C8.82688 8 9.5 7.4375 9.5 6.75C9.5 6.0625 8.82688 5.5 8 5.5C7.17313 5.5 6.5 6.0625 6.5 6.75V7C6.5 7.13261 6.44732 7.25979 6.35356 7.35355C6.25979 7.44732 6.13261 7.5 6 7.5C5.86739 7.5 5.74022 7.44732 5.64645 7.35355C5.55268 7.25979 5.5 7.13261 5.5 7V6.75C5.5 5.50938 6.62125 4.5 8 4.5C9.37875 4.5 10.5 5.50938 10.5 6.75C10.5 7.83625 9.64 8.74563 8.5 8.955Z"
                    fill="#B1B1B1"
                />
            </svg>
        </label>
        <textarea
            name="note"
            rows="5"
            v-model="formLesson.note"
            maxlength="10000"
        ></textarea>
        <UlError :message="formLesson.errors.note" />
        <ul class="note">
            <li>※ 宿題や教科書など補足情報</li>
        </ul>
    </div>
</template>
<style lang="scss" scoped>
@import "./_lesson.scss";

:deep(.modal) {
    .pop-up {
        width: 263px !important;
    }
}
</style>
