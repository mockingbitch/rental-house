...
const stateParent = useForm({
    parentVidPreview: props.formLesson.parentVidPreview,
    parentVidName: props.formLesson.parentVidName,
    short_video_for_parent_thumbnail:
        props.formLesson.short_video_for_parent_thumbnail,
    uploadValidate: "",
});
...
const snapImage = function (video, url) {
    ...
    if (success) {
        stateParent.short_video_for_parent_thumbnail = image;
        props.formLesson.short_video_for_parent_thumbnail = image;
    }

    return success;
};

const handleParentVidUpload = (event) => {
    ...
    reader.onload = () => {
        ...
        if (!allowedExtensions.exec(file.name)) {
            stateParent.uploadValidate =
                "ファイル形式が正しくありません。MP4、AVI、MOV、WEBM、FLV形式の画像をアップロードしてください";
        } else {
            stateParent.uploadValidate = "";
            stateParent.parentVidName = file.name;
            props.formLesson.parentVidName = file.name;
            const blob = new Blob([reader.result], { type: file.type });
            const url = URL.createObjectURL(blob);
            stateParent.parentVidPreview = url;
            props.formLesson.parentVidPreview = url;
...
const resetParentVid = () => {
    document.getElementById("true-file-upload-parent").value = "";
    stateParent.parentVidPreview = "";
    props.formLesson.short_video_for_parent = "";
    props.formLesson.parentVidPreview = "";
};
</script>
<template>
    <div
        class="file-name"
        v-if="props.formLesson.parentVidPreview"
        :style="{'background-image':`url(${props.formLesson.short_video_for_parent_thumbnail})`}"
        @click="isOpen = true"
    >
        <svg
            width="20"
        ...
        <span>{{
            props.formLesson.parentVidName.substring(0, 30) + "..."
        }}</span>
    </div>
    <label
        for="true-file-upload-parent"
    ...
    <input
        accept="video/*"
        type="file"
        @change="handleParentVidUpload"
        id="true-file-upload-parent"
        name="short_video_for_parent"
        hidden
        @input="formLesson.short_video_for_parent = $event.target.files[0]"
    />
    <UlError
        :message="props.message"
        v-show="stateParent.uploadValidate == ''"
    />
    <UlError :message="stateParent.uploadValidate" />
    <Popup
        :isOpen="isOpen"
        @close="isOpen = false"
        :state="stateParent"
        @resetParentVid="resetParentVid"
    />
</template>
<style lang="scss" scoped>
@import "./_parentVidPreview.scss";
</style>
